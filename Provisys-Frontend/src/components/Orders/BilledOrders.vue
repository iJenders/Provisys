...
const handleDeliverOrder = () => {
    ...
    axios.post(import.meta.env.VITE_API_URL + '/orders/deliver',
        { orderId: selectedOrder.value.id },
        {
            headers: { Authorization: `${localStorage.getItem('token')}` }
        }).then((res) => {
            successNotification("Pedido entregado", "El pedido ha sido entregado exitosamente")
            isSelectedOrder.value = false;
            getOrders()
        }).catch((err) => {
            handleRequestError(err);
        ...
        <el-dialog v-model="isSelectedOrder" title="Detalles del Pedido" width="80%" @close="selectedOrder = null"
            ...
            <template #footer>
                <div class="dialog-footer flex justify-end flex-wrap gap-4 gap-y-2">
                    ...
                    <el-button class="!ml-0" type="success" @click="handleDeliverOrder">
                        Entregar
                    </el-button>

                    <el-button class="!ml-0" type="danger" @click="handleCancelOrder">
                        Cancelar Pedido
                    </el-button>
                </div>
            </template>
