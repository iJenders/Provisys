...
const handleBillOrder = () => {
    ...
    axios.post(import.meta.env.VITE_API_URL + '/orders/bill',
        { orderId: selectedOrder.value.id },
        {
            headers: { Authorization: `${localStorage.getItem('token')}` }
        }).then((res) => {
            successNotification("Pedido facturado", "El pedido ha sido facturado exitosamente")
            isSelectedOrder.value = false;
            getOrders()
        }).catch((err) => {
        ...
        <el-dialog v-model="isSelectedOrder" title="Detalles del Pedido" width="80%" @close="selectedOrder = null"
            ...
            <template #footer>
                <div class="dialog-footer flex justify-end flex-wrap gap-4 gap-y-2">
                    ...
                    <el-button class="!ml-0" type="primary" @click="handleBillOrder">
                        Facturar
                    </el-button>

                    <el-button class="!ml-0" type="danger"
                        :disabled="fetchingModal || selectedOrder.status != '0' || (parseFloat(selectedOrder.payment.paid).toFixed(2) > 0)"
